"""
Lista de serviços anexa à Lei Complementar nº 116/2003 (galho NFS-e, referência).

DADO DE REFERÊNCIA, não extração: a NFS-e nacional traz o `cTribNac` (Código de
Tributação Nacional, 6 dígitos); seus 4 primeiros dígitos identificam o subitem
da lista da LC 116/2003 (ex.: "170201" → "17.02"). Aqui mapeamos cada subitem à
sua redação OFICIAL, transcrita do texto no Planalto, para EXIBIR ao operador a
descrição do código ao lado do número — pura apresentação (I-2: não altera nada
da extração, não decide tributação).

Subitem desconhecido → None (a tela mostra "descrição não localizada"; não se
inventa descrição — I-6). Anotações legislativas finais ("(Incluído pela…)",
"(Vide…)") foram removidas; parênteses da própria redação foram preservados.

Fonte: planalto.gov.br/ccivil_03/leis/lcp/lcp116.htm (Lista de serviços anexa).
"""
from __future__ import annotations

# subitem "N.NN" → redação oficial
LISTA_LC116: dict[str, str] = {
...
def subitem_de(codigo: str | None) -> str | None:
    """Deriva o subitem "N.NN" da LC 116 a partir do código do serviço.

    Aceita o cTribNac de 6 dígitos (ou qualquer string com ≥4 dígitos: item nos 2
    primeiros, subitem nos 2 seguintes) ou um subitem já no formato "N.NN".
    Devolve None se não conseguir derivar (ausência visível — I-6)."""
    if not codigo:
        return None
    dig = "".join(ch for ch in str(codigo) if ch.isdigit())
    if len(dig) >= 4:
        return f"{int(dig[:2])}.{dig[2:4]}"
    if "." in str(codigo):
        item, _, sub = str(codigo).partition(".")
        if item.isdigit() and sub[:2].isdigit():
            return f"{int(item)}.{sub[:2].zfill(2)}"
    return None


def descricao_servico(codigo: str | None) -> str | None:
    """Descrição oficial da LC 116 para o código do serviço, ou None se não
